"""
Run Wednesday/Tuesday 11 AM strategy backtest.

This is our data-driven strategy that emerged from exhaustive pattern analysis.
Tests the simple rule: Buy Wed/Tue at 11 AM, hold to EOD.
"""
...
from trading_playbook.core.wed_tue_11am_detector import detect_wed_tue_11am_signal
from trading_playbook.models.trade import Trade, BacktestResults, ExitReason


def run_wed_tue_backtest(fetcher, symbol, start_date, end_date, shares_per_trade=100, strategy_params=None):
    """Run backtest for Wednesday/Tuesday 11 AM strategy."""

    print(f"\n{'='*80}")
    print(f"WEDNESDAY/TUESDAY 11 AM BACKTEST: {symbol}")
    print(f"Period: {start_date} to {end_date}")
    print(f"Shares per trade: {shares_per_trade}")
    print(f"Strategy: ONLY trade Wed/Tue at 11 AM, hold to EOD")
    print(f"{'='*80}\n")
    ...
    for i, trade_date in enumerate(trading_days, 1):
        ...
        # Detect signal
        signal = detect_wed_tue_11am_signal(df, strategy_params)
...
def calculate_returns_on_investment(results, initial_capital=10000):
    ...
    # Calculate how many shares we can buy with initial capital
    # Use first entry price as reference
    first_entry_price = results.trades[0].entry_price
    shares_we_can_buy = int(initial_capital / first_entry_price)

    # Calculate total P&L scaled to our position size
    # All trades are 100 shares, scale to our actual shares
...
def main():
    ...
    if results.trades:
        ...
        print("="*80)
        print("\nDP20 (Sep-Nov):              -$874.72, 6.7% win rate")
        print(f"Morning Reversal (6mo):      +${398:.2f}, 51.0% win rate")
        print(f"Wed/Tue 11AM (6mo):          +${results.total_pnl:,.2f}, {results.win_rate:.1f}% win rate")

        if results.total_pnl > 398:
            improvement = results.total_pnl - 398
            print(f"\n🎉 Wed/Tue 11AM beats Morning Reversal by ${improvement:,.2f}!")

        if results.total_pnl > 0:
            print(f"\n✅ This strategy is PROFITABLE over 6 months")
        else:
            print(f"\n❌ This strategy still loses money")
